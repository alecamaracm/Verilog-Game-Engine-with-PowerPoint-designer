            ...
            Task.Run(() => {
                ...
                using (StreamWriter writer = new StreamWriter(currentProjectFolderPath + "\\IH8Verilog_main.v"))
                {
                    ...
                    foreach (MemoryBitmaps m in bitmapsToWrite)
                    {
                        ...
                        writer.WriteLine(String.Format("ram{0} {0}ram((((yPixel-{0}Y+{0}SKIPY)/{2})*{1} +((xPixel-{0}X)/{2})+1)+({0}SKIPX/{3}),CLOCK,16'd0,0,{0}q);",
                            m.name,m.reducedBitmap.Width,m.scale,skipscale[m.name]));
                    ...
                    if (picture.DrawingType == DrawingType.Picture)
                    {
                        ...
                        if (properties.ContainsKey("MEMORY"))
                        {
                            ...
                            if (properties.ContainsKey("SKIPX"))
                            {
                                ...
                                {
                                    writer.WriteLine(getTabs(tabs) + String.Format("if(" + extraaaaaa + "yPixel>={0}Y && yPixel<{0}Y+({2}*{1})-{4}SKIPY && xPixel>={0}X+1 && xPixel<{0}X+({3}*{1}-{4}SKIPX) && {0}q[15]==1'b1)", properties["NAME"].Replace(" ", "_"), compresionFactor, (int)(picture.Layout.Height / compresionFactor), (int)(picture.Layout.Width / compresionFactor), properties["NAME"]));

                                }
                                else
                                {
                                    writer.WriteLine(getTabs(tabs) + String.Format("if(" + extraaaaaa + "yPixel>={0}Y && yPixel<{0}Y+({2}*{1}) && xPixel>={0}X+1 && xPixel<{0}X+({3}*{1}-{4}SKIPX) && {0}q[15]==1'b1)", properties["NAME"].Replace(" ", "_"), compresionFactor, (int)(picture.Layout.Height / compresionFactor), (int)(picture.Layout.Width / compresionFactor), properties["NAME"]));
