...
namespace PowerPointVerilogEngineDesigner
{
    public partial class Form1 : Form
    {
        ...
        List<string> skipX = new List<string>();

        List<string> spriteCount = new List<string>();

        List<string> visibles = new List<string>();
        bool working = false;

        Dictionary<string, int> skipscale = new Dictionary<string, int>();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() => {
                ...
                using (StreamWriter writer = new StreamWriter(currentProjectFolderPath + "\\IH8Verilog_main.v"))
                {
                    ...
                    foreach (String s in spriteCount)
                    {
                        writer.WriteLine("input [9:0]" + s + ";");
                    }
                    ...
                    foreach (MemoryBitmaps m in bitmapsToWrite)
                    {
                        ...
                        if (m.spriteCount>1)
                        {
                            extraa = "+"+(m.size / m.spriteCount).ToString() + "*"+m.name+"sp";
                        }
        ...
        private void writeAnimationModule(Slide slide,StreamWriter writer)
        {
           string aaa=("\nmodule animations(animationCLOCK,reset,wasd,arrows," +  String.Join(",", bouncings.ToArray())+","+ String.Join(",",arrowMovings.ToArray())+","+ String.Join(",", wasdMovings.ToArray()) + ");\n");
            ...
            foreach (String s in bouncings)
            {
                ...
                if (s.EndsWith("X"))
                {
                    writer.WriteLine("\t\tif (" + s + " <= " + properties.minX + ")");
                }
                else
                {
        ...
        private void drawBasicThingys(StreamWriter writer, Slide slide, int tabs)
        {
            ...
            foreach (GeometryShape picturex in slide.Content.Drawings)
            {
                ...
                int spriteCount = 1;
                if (properties.ContainsKey("SPRITES")) spriteCount = int.Parse(properties["SPRITES"]);
                ...
                {                    
                    ...
                    if (properties.ContainsKey("WASDMovement") && properties.ContainsKey("NAME") && properties.ContainsKey("MOVEABLE"))
                    {
                        ...
                        movingProperties.Add(properties["NAME"].Replace(" ", "_"), new MovingProperties() { height = (int)picturex.Layout.Height.To(LengthUnit.Point), width = (int)picturex.Layout.Width.To(LengthUnit.Point)/ spriteCount, name = properties["NAME"].Replace(" ", "_"), speed = sp,minX=minX,maxX=maxX,maxY=maxY,minY=minY });
                    }
                ...
                if (picturex is Picture)
                {
                    ...
                    if (picture.DrawingType == DrawingType.Picture)
                    {
                        ...
                        if (properties.ContainsKey("MEMORY"))
                        {
                            ...
                            memoryBitmaps.spriteCount = spriteCount;
                            ...
                            skipscale.Add(properties["NAME"], compresionFactor);

                            if (properties.ContainsKey("SKIPX"))
                            {
                           
                                if (properties.ContainsKey("SKIPY"))
                                {
                                    writer.WriteLine(getTabs(tabs) + String.Format("if(" + extraaaaaa + "yPixel>={0}Y && yPixel<{0}Y+({2}*{1})-{4}SKIPY && xPixel>={0}X+1 && xPixel<{0}X+({3}*{1}-{4}SKIPX) && {0}q[15]==1'b1)", properties["NAME"].Replace(" ", "_"), compresionFactor, (int)(picture.Layout.Height / compresionFactor), (int)(picture.Layout.Width / compresionFactor)/ spriteCount, properties["NAME"]));

                                }
                                else
                                {
                                    writer.WriteLine(getTabs(tabs) + String.Format("if(" + extraaaaaa + "yPixel>={0}Y && yPixel<{0}Y+({2}*{1}) && xPixel>={0}X+1 && xPixel<{0}X+({3}*{1}-{4}SKIPX) && {0}q[15]==1'b1)", properties["NAME"].Replace(" ", "_"), compresionFactor, (int)(picture.Layout.Height / compresionFactor), (int)(picture.Layout.Width / compresionFactor)/ spriteCount, properties["NAME"]));
                            ...
                            else
                            {
                                if (properties.ContainsKey("SKIPY"))
                                {
                                    writer.WriteLine(getTabs(tabs) + String.Format("if(" + extraaaaaa + "yPixel>={0}Y && yPixel<{0}Y+({2}*{1})-{4}SKIPY && xPixel>={0}X+1 && xPixel<{0}X+({3}*{1}) && {0}q[15]==1'b1)", properties["NAME"].Replace(" ", "_"), compresionFactor, (int)(picture.Layout.Height / compresionFactor), (int)(picture.Layout.Width / compresionFactor)/ spriteCount, properties["NAME"]));

                                }
                                else
                                {
                                    writer.WriteLine(getTabs(tabs) + String.Format("if(" + extraaaaaa + "yPixel>={0}Y && yPixel<{0}Y+({2}*{1}) && xPixel>={0}X+1 && xPixel<{0}X+({3}*{1}) && {0}q[15]==1'b1)", properties["NAME"].Replace(" ", "_"), compresionFactor, (int)(picture.Layout.Height / compresionFactor), (int)(picture.Layout.Width / compresionFactor)/spriteCount));
        ...
        public List<string> getMovableVarNames(Slide slide)
        {
            ...
            foreach (Picture shape in slide.Content.Drawings.OfType<Picture>())
            {
                ...
                if (properties.ContainsKey("SKIPX") && properties.ContainsKey("NAME"))
                {
                    skipX.Add(properties["NAME"].Replace(" ", "_") + "SKIPX");
                }
                if (properties.ContainsKey("VISIBLE") && properties.ContainsKey("NAME"))
                {
                    skipY.Add(properties["NAME"].Replace(" ", "_") + "VISIBLE");
                }
                if (properties.ContainsKey("SPRITES") && properties.ContainsKey("NAME"))
                {
                    skipY.Add(properties["NAME"].Replace(" ", "_") + "sp");
                }
            }
