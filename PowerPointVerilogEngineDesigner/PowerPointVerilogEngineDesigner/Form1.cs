...
namespace PowerPointVerilogEngineDesigner
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() => {
                ...
                using (StreamWriter writer = new StreamWriter(currentProjectFolderPath + "\\IH8Verilog_main.v"))
                {
                    ...
                    writer.WriteLine("module PP2VerilogDrawingController(animationCLOCK,wasd,arrows,xPixel,yPixel,VGAr,VGAg,VGAb,mouseX,mouseY," + String.Join(",",movs.ToArray())+");" + Environment.NewLine);
                    ...
                    {
                        writer.WriteLine("\nanimations anim1(animationCLOCK,wasd,arrows," + String.Join(",", arrowMovings.ToArray()) + "," + String.Join(",", wasdMovings.ToArray()) + ");");
                    }
        ...
        {
            writer.WriteLine("\nmodule animations(animationCLOCK,wasd,arrows," + String.Join(",",arrowMovings.ToArray())+","+ String.Join(",", wasdMovings.ToArray()) + ");\n");
                ...
                if(shape.Format.Fill.FillType==FillFormatType.Solid)
                {
                    ...
                    if (shape.ShapeType==ShapeGeometryType.Rectangle)
                    {
                        ...
                        if (properties.ContainsKey("WASDMovement") && properties.ContainsKey("NAME") && properties.ContainsKey("MOVEABLE"))
                        {
                            ...
                            wasdMovings.Add(properties["NAME"]+"X");
                            wasdMovings.Add(properties["NAME"] + "Y");
                            movingProperties.Add(properties["NAME"], new MovingProperties() { height= (int)shape.Layout.Height.To(LengthUnit.Point), width= (int)shape.Layout.Width.To(LengthUnit.Point), name=properties["NAME"],speed=sp});
                        }
                          
                        if (properties.ContainsKey("ARROWSMovement") && properties.ContainsKey("NAME") && properties.ContainsKey("MOVEABLE"))
                        {
                            int sp = properties.ContainsKey("AnimationSpeed") ? int.Parse(properties["AnimationSpeed"]) : 1;
                            arrowMovings.Add(properties["NAME"]+"X");
                            arrowMovings.Add(properties["NAME"] + "Y");
                            movingProperties.Add(properties["NAME"], new MovingProperties() { height = (int)shape.Layout.Height.To(LengthUnit.Point), width = (int)shape.Layout.Width.To(LengthUnit.Point), name = properties["NAME"], speed = sp });
        ...
        public List<string> getMovableVarNames(Slide slide)
        {
            ...
            {
                Dictionary<string, string> properties = getProperties(shape.Text);
                if(properties.ContainsKey("MOVEABLE") && properties.ContainsKey("NAME") && properties.ContainsKey("WASDMovement")==false && properties.ContainsKey("ARROWSMovement")==false)
                {
